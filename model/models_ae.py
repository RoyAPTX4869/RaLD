...
class Attention(nn.Module):
    ...
    def forward(self, x, context = None, mask = None):
        ...
        if exists(mask):
            # sim = sim + mask
            sim = sim.masked_fill(mask != 0, -torch.inf)
...
class AutoEncoder(nn.Module):   # not actually used
    def __init__(
        self,
        *,
        depth=24,             
...
class KLAutoEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        weight_tie_layers = False,
        decoder_ff = False,
        query_type='point'  # 'point' or 'learnable' or 'mix'
    ):
        super().__init__()
        ...
        if query_type == 'point':
            ...
            self.s_latents = nn.Embedding(num_latents, dim)
            self.d_latents = nn.Embedding(num_latents, dim)
            self.mix_attn_layer =  get_latent_attn(**cache_args)
    ...
    def encode(self, pc):
        ...
        if self.query_type == 'point':
            ...
            ### sort sampled_pc by the sum of all channel (x-axis + y-axis + z-axis)
        ...
        elif self.query_type == 'mix':
            ##################### mix query v2 ###################
            static_query = repeat(self.s_latents.weight, 'n d -> b n d', b=B)
            dynamic_query = repeat(self.d_latents.weight, 'n d -> b n d', b=B)
            dynamic_query = self.mix_attn_layer(dynamic_query, context = pc_embeddings, mask = None)
            x = static_query + dynamic_query
            x = self.query_proj(x)
            ##################### mix query v2 end ###############
        else:
...
def create_autoencoder(dim=512, M=512, latent_dim=64, N=2048, determinisitc=False, query_type='point'):
    if determinisitc:
        model = AutoEncoder(
            depth=24,
            dim=dim,
...
def kl_d512_m512_l32_learn(N=2048):
    return create_autoencoder(dim=512, M=512, latent_dim=32, N=N, determinisitc=False, query_type='learnable')

def kl_d512_m512_l32_mix(N=2048):
    return create_autoencoder(dim=512, M=512, latent_dim=32, N=N, determinisitc=False, query_type='mix')

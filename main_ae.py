...
def main(args):
    ...
    dataset_val.set_load_radar(False)
    ...
    # if global_rank == 0 and args.system.log_dir is not None and not args.train.eval:
    ...
    model = models_ae.__dict__[args.lidar_ae.name](N=args.lidar_ae.point_cloud_size)
    model.to(device)
    ...
    model_params, ema_params = misc.load_model(args=args, 
                                               ...
                                               ema=args.system.mode == 'train',device=device)

    if args.system.mode == 'eval':
        test_stats = evaluate(data_loader_val, model_without_ddp, device,args=args)
    ...
    for epoch in range(args.train.start_epoch, args.train.epochs):
        ...
        # save checkpoint
        if args.system.output_dir and (epoch % 10 == 0 or epoch + 1 == args.train.epochs):
            misc.save_model(
